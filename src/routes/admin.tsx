...
function Admin() {
  ...
  useSocketEvent(SOCKET_EVENTS.SERIES_CREATED, () =>
    dispatch(fetchMoviesThunk()),
  );
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              label="Total Revenue"
              value={
                stats?.totalRevenue ? formatRevenue(stats.totalRevenue) : "—"
              }
              hint="All time"
              icon={DollarSign}
            />
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-card">
            <div className="flex items-center justify-between mb-6">
              ...
              <Badge
                variant="outline"
                className="text-xs text-success border-success/40 gap-1"
              >
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueData}>
                  ...
                  <CartesianGrid
                    strokeDasharray="3 3"
                    stroke="oklch(0.22 0.016 258)"
                    vertical={false}
                  />
                  <XAxis
                    dataKey="month"
                    stroke="oklch(0.68 0.012 258)"
                    fontSize={11}
                    tickLine={false}
                    axisLine={false}
                  />
            ...
            {[
              {
                ...
                value: `${movies.filter((m) => m.status === "published").length} titles`,
              ...
              >
                <span
                  className={`inline-flex size-10 shrink-0 items-center justify-center rounded-xl ${s.color}`}
                >
              ...
              const count =
                card.href === "/admin/categories"
                  ...
                  : card.href === "/admin/movies"
                    ...
                    : card.href === "/admin/banner"
                      ? `${movies.filter((m) => m.is_banner).length} active banner movies`
                      : "";
              return (
                <div
                  key={card.title}
          ...
          <div className="rounded-2xl border border-border/60 bg-card shadow-card overflow-hidden">
            <div className="flex items-center justify-between border-b border-border/60 px-6 py-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  All payment records from Stripe
                </p>
              </div>
              <Badge
                variant="outline"
            ...
            {paymentsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Payments appear here after a successful Stripe checkout.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/60 text-left">
                      <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70">
                        User
                      </th>
                      <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70">
                        Plan
                      ...
                      <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70 hidden lg:table-cell">
                        Payment Intent
                      </th>
                    </tr>
                  </thead>
        ...
        <TabsContent value="activity" className="space-y-4">
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-card">
            <div className="mb-6">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Recent payment events in real-time
              </p>
            </div>
            {payments.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {payments.slice(0, 10).map((p) => (
                  ...
                  >
                    <div
                      className={`mt-0.5 size-2.5 shrink-0 rounded-full ring-2 ring-offset-2 ring-offset-card ${p.status === "succeeded" ? "bg-success ring-success/30" : "bg-destructive ring-destructive/30"}`}
                    />
